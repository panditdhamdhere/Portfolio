    ...
    <>
      <div className='flex justify-between items-center w-full h-20 px-4 text-white bg-black fixed'>
        ...
        <ul className='hidden md:flex '>
          {links.map(({ id, link }) => (
            <li key={id}
              className='px-4 cursor-pointer capitalize font-medium text-lg text-blue-500 hover:scale-110 duration-200'>{link}</li>
          ))}
        </ul>
        ...
        </div>
        {
          nav && (
            <ul className='flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from black to bg-gray-800 text-blue-500'>
              {links.map(({ id, link }) => (
                <li key={id}
                  className='px-4 cursor-pointer  py-6 text-4xl capitalize'>{link}</li>
              ))}
            </ul>
          )}
